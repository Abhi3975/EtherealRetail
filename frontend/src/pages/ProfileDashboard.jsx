...
const ProfileDashboard = () => {
  ...
  useEffect(() => {
    // In a real app, JWT auth protects this
    api.get('/orders/list')
      .then(res => {
        if(res.data.success) {
      ...
      .catch(err => {
        // Fallback for demo when DB is empty
        setOrders([
           { _id: 'ord_demo_992', createdAt: new Date().toISOString(), financials: { grandTotal: 105 }, paymentStatus: 'paid' }
        ]);
      });
  }, []);
    ...
    <div className="bg-cloud-dancer min-h-screen pt-40 pb-32 px-8 lg:px-20">
      <div className="max-w-screen-2xl mx-auto">
        <header className="mb-20">
            <span className="font-sans uppercase tracking-[0.5em] text-[10px] text-stone-gray font-bold">Authenticated Curator</span>
            <h1 className="text-5xl md:text-7xl font-serif text-luxury-black mt-4 italic">The Archives</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-24">
            ...
            <div className="md:col-span-2 space-y-12">
                <h2 className="text-3xl font-serif italic text-luxury-black">Your Past Expressions</h2>
                ...
                {orders.length === 0 ? (
                    <div className="bg-white/20 border border-stone-gray/10 p-20 text-center space-y-8 backdrop-blur-md">
                        ...
                        <p className="font-serif italic text-xl text-stone-gray">Your history is a blank canvas.</p>
                        ...
                        {orders.map(order => (
                            ...
                            >
                                <div className="space-y-2 mb-6 md:mb-0">
                                    <p className="font-mono text-[10px] tracking-widest text-stone-gray group-hover:text-white/40 uppercase">Selection #{order._id.substring(0, 12)}</p>
                                    <div className="flex items-baseline gap-4">
                                        <p className="font-serif text-2xl italic leading-none">{new Date(order.createdAt).toLocaleDateString()}</p>
                                        <span className="w-1 h-1 bg-stone-gray rounded-full" />
                                        <p className="font-sans font-bold text-lg tracking-widest">${order.financials?.grandTotal || 0}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-8 w-full md:w-auto">
                                    <span className={`text-[10px] uppercase font-bold tracking-widest px-6 py-3 border ${order.paymentStatus === 'paid' ? 'border-accent-neon text-stone-gray bg-accent-neon/5 group-hover:bg-accent-neon/20 group-hover:text-accent-neon' : 'border-stone-gray/20 text-stone-gray group-hover:text-white/40'}`}>
                                        {order.paymentStatus}
                                    </span>
