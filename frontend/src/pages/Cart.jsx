    ...
    <div className="bg-cloud-dancer min-h-screen pt-40 pb-20 px-8 lg:px-20">
      <div className="max-w-screen-2xl mx-auto">
        <header className="mb-20 text-center">
            <span className="font-sans uppercase tracking-[0.5em] text-[10px] text-stone-gray font-bold">Secure Bag</span>
            <h1 className="text-5xl md:text-7xl font-serif text-luxury-black mt-4 italic">Your Curation</h1>
        </header>

        {items.length === 0 ? (
            <div className="text-center py-40 border-y border-stone-gray/20">
                <p className="font-serif text-2xl italic text-stone-gray mb-8">Your bag is weightless.</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-24">
                            ...
                            >
                                ...
                                <div className="flex-1 flex flex-col justify-between">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="font-serif text-2xl md:text-3xl text-luxury-black mb-1">{item.product.title}</h3>
                                            <p className="font-sans text-[10px] uppercase tracking-widest font-bold text-stone-gray mb-4">{item.product.subCategory} / {item.variant}</p>
                                        </div>
                                        <p className="font-sans font-bold text-lg tracking-widest">${item.product.basePrice * item.quantity}</p>
                                    </div>
                                    ...
                                    <div className="flex justify-between items-center text-luxury-black">
                                        <div className="flex items-center border border-stone-gray/30">
                                            ...
                                            ><FiMinus /></button>
                                            <span className="px-4 font-sans font-bold text-sm tracking-widest">{item.quantity}</span>
                                            <button 
                                                onClick={() => updateQuantity(item.productId, item.variant, 1)}
                                                className="px-4 py-2 hover:bg-stone-gray/10 transition"
                                            ><FiPlus /></button>
                ...
                <div className="space-y-12">
                    <div className="bg-white/40 backdrop-blur-3xl border border-white/60 p-12 space-y-10">
                        ...
                        <div className="space-y-4 font-sans text-xs uppercase tracking-widest font-bold text-stone-gray">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="text-luxury-black font-serif italic normal-case tracking-normal">Free</span>
                            </div>
                    ...
                    <div className="p-8 border border-dashed border-stone-gray/20 rounded-2xl space-y-4 text-center">
                        <p className="font-serif italic text-stone-gray text-sm">Every acquisition is a commitment to archival quality.</p>
                        <p className="font-sans text-[8px] uppercase tracking-[0.3em] font-bold text-stone-gray/40">Secure Encryption Enabled</p>
                    </div>
                </div>
            </div>
        )}
