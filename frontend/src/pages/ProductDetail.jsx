...
const ProductDetail = ({ setCartCount }) => {
  ...
  useEffect(() => {
    api
      ...
      .catch((err) =>
        toast.error("Could not find this masterpiece.", { theme: "dark" }),
      );
  }, [id]);
        ...
        <div className="w-full lg:w-[45%] space-y-12">
          <header className="space-y-6 pb-12 border-b border-stone-gray/20">
            <div className="flex items-center gap-4 text-stone-gray font-sans text-xs uppercase font-bold tracking-[0.4em]">
              ...
              <span className="italic normal-case tracking-normal font-serif text-sm opacity-60">
                Architectural Series
              </span>
            </div>
            <h1 className="text-6xl md:text-8xl font-serif text-luxury-black leading-[0.9]">
              {product.title}
            </h1>
            <p className="text-3xl font-serif italic text-luxury-black/60">
              ${product.basePrice}
            </p>
          </header>
          ...
          <div className="space-y-10">
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-end">
                <span className="font-sans font-bold uppercase tracking-[0.3em] text-[10px]">
                  Select Fit
                </span>
                <button className="text-[10px] uppercase font-bold tracking-widest text-stone-gray underline decoration-stone-gray/30 underline-offset-4">
                  Fit Guide
                </button>
              </div>
            ...
            <div className="flex flex-col gap-4 pt-4">
              ...
              <div className="flex items-center justify-center gap-4 py-4 text-stone-gray/60 font-sans text-[8px] uppercase font-bold tracking-[0.2em] border-t border-stone-gray/10">
                <span className="flex items-center gap-2">
                  <FiTruck /> Global Express
                </span>
                <span className="flex items-center gap-2">
                  <FiShield /> Authenticated Build
                </span>
                <span className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {activeTab === tab.id && (
                      <motion.div
                        ...
                        className="absolute bottom-0 left-0 w-full h-[2px] bg-luxury-black"
                      />
                    )}
                  </button>
                ))}
                ...
                >
                  {activeTab === "story" && (
                    <>
                      <p>
                        {product.description ||
                          "Designed with absolute intent, this piece represents the pinnacle of modern functional attire. Our process involves thousands of iterations to achieve perfect drape and utility."}
                      </p>
                      <ul className="space-y-2 pt-4">
                        {product.brandHighlights?.map((h, i) => (
                          <li
                            key={i}
                            className="flex items-center gap-3 italic font-serif text-luxury-black"
                          >
                  ...
                  {activeTab === "specs" && (
                    <div className="grid grid-cols-2 gap-y-6">
                      <div>
                        <h5 className="font-bold uppercase text-[10px] tracking-widest mb-1">
                          Material
                        </h5>
                        <p>100% Architecture Grade</p>
                      </div>
                      <div>
                        <h5 className="font-bold uppercase text-[10px] tracking-widest mb-1">
                          Weight
                        </h5>
                        <p>Medium Lifestyle</p>
                      </div>
                      <div>
                        <h5 className="font-bold uppercase text-[10px] tracking-widest mb-1">
                          Traceability
                        </h5>
                        <p>Tier 1 Transparency</p>
                      </div>
                      <div>
                        <h5 className="font-bold uppercase text-[10px] tracking-widest mb-1">
                          Assembly
                        </h5>
                        <p>Global Craftship</p>
                      </div>
                    </div>
                  )}
                  {activeTab === "shipping" && (
                    <p>
                      All items are shipped via high-priority secure channels.
                      Returns are accepted within 7 days of arrival in pristine
                      original condition. Our concierge team is available 24/7
                      for collection fitment assistance.
                    </p>
                  )}
                </motion.div>
              </AnimatePresence>
