  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        navigate('/profile');
        window.location.reload(); // Quick sync for Navbar context
      }
    } catch (err) {
      toast.error(err.response?.data?.message || "Authentication failed. Access Denied.", { theme: "dark" });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="bg-cloud-dancer min-h-screen pt-40 pb-20 px-8 flex items-center justify-center">
      ...
      >
        <header className="text-center space-y-4">
          <span className="font-sans uppercase tracking-[0.6em] text-[10px] text-stone-gray font-bold">Atelier Credentials</span>
          <h1 className="text-5xl md:text-6xl font-serif text-luxury-black">
            {isLogin ? "Sign In" : "Register"}
          </h1>
          <p className="font-serif italic text-stone-gray">Access the exclusive archival collection.</p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <AnimatePresence mode="wait">
            {!isLogin && (
              <motion.div 
                ...
                className="space-y-2"
              >
                <label className="font-sans text-[10px] uppercase tracking-widest font-bold text-stone-gray">Full Identity</label>
                <input 
                  type="text"
                  required
          ...
          <div className="space-y-2">
            <label className="font-sans text-[10px] uppercase tracking-widest font-bold text-stone-gray">Encryption Gateway (Email)</label>
            <input 
              type="email"
              required
          ...
          <div className="space-y-2">
            <label className="font-sans text-[10px] uppercase tracking-widest font-bold text-stone-gray">Secret Key (Password)</label>
            <input 
              type="password"
              required
              placeholder="••••••••"
          ...
          >
            {loading ? "Decrypting..." : isLogin ? "Authorize Access" : "Create Masterpiece Account"}
          </button>
        </form>
        ...
        <footer className="text-center pt-8 border-t border-stone-gray/10">
          ...
          >
            {isLogin ? "No Credentials? Register Your Presence" : "Already Recognized? Sign In"}
          </button>
        </footer>
      </motion.div>
    </div>
