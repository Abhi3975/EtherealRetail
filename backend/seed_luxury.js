...
// Category-specific apparel datasets with strict sub-category mapping
...
const subCategories = ["Topwear", "Bottomwear", "Outerwear", "Accessories", "Footwear", "Dresses"];
...
const getStrictSubCategorySnippet = (category, index) => {
    const dataset = category === "Men" ? fashion_datasets.mens : fashion_datasets.womens;
...
const luxuryProducts = [
  // Flagship items with strictly hand-picked valid images and subcategories
  {
    title: "Atelier Ethereal Cashmere Overcoat",
    description: "The definitive masterpiece of the Atelier. Italian cashmere with a structured shoulder.",
    basePrice: 1250,
    category: "Men",
        ...
        for (const [subCategory, imageUrls] of Object.entries(categoriesObj)) {
            for (const imageUrl of imageUrls) {
                ...
                extras.push({
                    ...
                    active: true,
                    isFeatured: i % 4 === 0, // Ensure healthy amount of features without duplicates
...
const seed = async () => {
  try {
    ...
    console.log("Connected to Atlas for Phase 5: Categorization & Scaling (350+).");
    await Product.deleteMany({});
    console.log("Old catalog purged.");
    const finalSet = [...luxuryProducts, ...generateExtraProducts()];
    await Product.insertMany(finalSet);
    console.log(`Successfully seeded ${finalSet.length} strictly categorized masterpieces.`);
    process.exit(0);
  } catch (err) {
