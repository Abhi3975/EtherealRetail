...
router.post('/place', protectRoute, async (req, res) => {
  ...
  if (!lineItems || lineItems.length === 0) {
    return res.status(400).json({ success: false, message: "Trying to buy air? The cart is empty." });
  }

  try {
    const subtotal = lineItems.reduce((acc, item) => acc + (item.price * item.quantity), 0);
    const grandTotal = subtotal; // Complimentary shipping for Atelier curated items

    console.log(`Processing an authenticated order for $${grandTotal}...`);
    
    // Grab the real User ID from the req.user object attached by protectRoute
    ...
    const orderRecord = new Order({
      ...
      paymentStatus: paymentMethod === 'cod' ? 'cod_pending' : 'pending_gateway',
    ...
    // Stripe or Razorpay placeholder
    return res.json({
      success: true,
...
router.get('/list', protectRoute, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Could not fetch orders", error: err.message });
  }
});
